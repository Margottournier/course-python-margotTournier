# -*- coding: utf-8 -*-
"""
Created on Sun Sep 24 18:01:22 2023
...
# Function to check the state of an element at a given temperature
def find_the_element(symbol, temperature):
    symbol = symbol.capitalize()  # Ensure the symbol is in uppercase
    element = dico_element.get(symbol)
    ...
    if element:
        ...
        if temperature_K < melting_point:
            return f"{element['name']} is solid at {temperature_K} K."
...
result = find_the_element(element_symbol, temperature_K)
print(result)
